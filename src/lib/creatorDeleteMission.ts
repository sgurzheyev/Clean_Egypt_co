...
/** Creator-only delete for $0 / unfunded pins. Funded rows are rejected by RPC + RLS. */
export async function creatorDeleteMission(missionId: string): Promise<void> {
  const { error } = await supabase.rpc('creator_delete_mission', {
