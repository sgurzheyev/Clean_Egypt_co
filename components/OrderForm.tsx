...
const OrderForm: React.FC<OrderFormProps> = ({ mode, language }) => {
  ...
  return (
    <div className="w-full max-w-lg p-1 bg-gradient-to-br from-[#39FF14] to-[#BC13FE] rounded-[2.5rem]">
      <form onSubmit={handleSubmit} className="p-8 space-y-4 bg-black rounded-[2.4rem] text-white">
        <h2 className="text-2xl font-black italic text-[#39FF14] uppercase">New Mission</h2>
        
        <input type="email" placeholder="Email" required className="w-full p-4 bg-[#111] border border-white/10 rounded-2xl outline-none focus:border-[#39FF14]" value={email} onChange={e => setEmail(e.target.value)} />
        
        <div className="flex gap-2">
          <input type="text" placeholder="Name" required className="w-1/2 p-4 bg-[#111] border border-white/10 rounded-2xl outline-none focus:border-[#39FF14]" value={clientName} onChange={e => setClientName(e.target.value)} />
          <input type="tel" placeholder="Phone" required className="w-1/2 p-4 bg-[#111] border border-white/10 rounded-2xl outline-none focus:border-[#BC13FE]" value={phone} onChange={e => setPhone(e.target.value)} />
        </div>
        ...
        <textarea placeholder="Mission details..." className="w-full p-4 bg-[#111] border border-white/10 rounded-2xl outline-none" rows={2} value={comment} onChange={e => setComment(e.target.value)} />
        
        <button type="submit" disabled={isSubmitting} className="w-full py-5 rounded-2xl bg-gradient-to-r from-[#39FF14] to-[#BC13FE] text-black font-black uppercase italic active:scale-95 transition-all">
