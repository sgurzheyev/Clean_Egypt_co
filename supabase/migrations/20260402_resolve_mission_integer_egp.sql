-- Whole EGP payouts: floor() splits so wallet/transaction amounts satisfy integer checks (check_integer_egp).
...
begin
  ...
  if lower(coalesce(p_decision, '')) = 'approve' then
    ...
    if coalesce(p_supervisor_verified, false) and p_supervisor_user_id is not null then
      ...
      v_debug_note := format(
        'ESCROW_DEBUG funding_floor=%s cleaner_reward=%s scout_reward=%s supervisor_bounty=%s platform_company_half=%s raw_current_funding=%s raw_amount_target=%s mission_id=%s supervisor=%s',
        v_escrow_debit,
        v_cleaner_reward,
        v_scout_reward,
        v_supervisor_reward,
      ...
      v_debug_note := format(
        'ESCROW_DEBUG funding_floor=%s cleaner_reward=%s scout_reward=%s platform_fee_remainder=%s raw_current_funding=%s raw_amount_target=%s mission_id=%s',
        v_escrow_debit,
        v_cleaner_reward,
        v_scout_reward,
        v_escrow_debit - v_cleaner_reward - v_scout_reward,
        v_raw_current_funding,
        v_raw_amount_target,
        p_mission_id
      );
    ...
    v_skip_frozen := v_raw_current_funding >= v_funding - 0.01;

    if not v_skip_frozen then
      ...
      update public.profiles
        set frozen_balance = coalesce(frozen_balance, 0) - v_escrow_debit
      where id = v_mission.creator_id;
    end if;
